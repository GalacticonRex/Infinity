#ifndef __INFI_VECTOR_BASE_H__
#define __INFI_VECTOR_BASE_H__
...
#define FULLCTOR( n, a,b,c,d ) \
	...
	vec##n##_t(std::initializer_list<TEMPLATE_NAME> args) { \
		InfiPushFunction( "vec_" #n "_t.ctor(initializer_list)" ); \
		if ( args.end() - args.begin() != n ) \
			InfiSendError( INFI_INIT_ERROR, \
						   "invalid number of elements (got %d, need %d)", args.end() - args.begin(), n ); \
...
TEMP1 union vec2_t {
	...
	struct { TEMPLATE_NAME data[2]; };
	struct { TEMPLATE_NAME x,y; };
	struct { TEMPLATE_NAME r,g; };
	struct { TEMPLATE_NAME s,t; };
	struct { TEMPLATE_NAME u,v; };
	
	struct { TEMPLATE_NAME red,green; };
	struct { TEMPLATE_NAME column,row; };
	...
	PERMUTE2(r,g,OPERATION2,OPERATION3,OPERATION4)
	PERMUTE2(s,t,OPERATION2,OPERATION3,OPERATION4)
	PERMUTE2(u,v,OPERATION2,OPERATION3,OPERATION4)
	
};

TEMP1 union vec3_t {

	struct { TEMPLATE_NAME data[3]; };
	struct { TEMPLATE_NAME x,y,z; };
	struct { TEMPLATE_NAME r,g,b; };
	struct { TEMPLATE_NAME h,s,v; };
	
	struct { TEMPLATE_NAME red,green,blue; };
	struct { TEMPLATE_NAME right,up,forward; };
	struct { TEMPLATE_NAME hue,saturation,value; };	
	...
	PERMUTE3(r,g,b,OPERATION2,OPERATION3,OPERATION4)
	
};

TEMP1 union vec4_t {

	struct { TEMPLATE_NAME data[4]; };
	struct { TEMPLATE_NAME x,y,z,w; };
	struct { TEMPLATE_NAME r,g,b,a; };
	
	struct { TEMPLATE_NAME red,green,blue,alpha; };
	...
	PERMUTE4(r,g,b,a,OPERATION2,OPERATION3,OPERATION4);
	
};

PERMUTE2(x,y,OPERATION22,OPERATION23,OPERATION24);
PERMUTE2(r,g,OPERATION22,OPERATION23,OPERATION24);
PERMUTE2(s,t,OPERATION22,OPERATION23,OPERATION24);
PERMUTE2(u,v,OPERATION22,OPERATION23,OPERATION24);

PERMUTE3(x,y,z,OPERATION32,OPERATION33,OPERATION34);
PERMUTE3(r,g,b,OPERATION32,OPERATION33,OPERATION34);

PERMUTE4(x,y,z,w,OPERATION42,OPERATION43,OPERATION44);
PERMUTE4(r,g,b,a,OPERATION42,OPERATION43,OPERATION44);
