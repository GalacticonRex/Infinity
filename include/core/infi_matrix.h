#ifndef __INFI_MATRIX_N_X_N_234_H__
#define __INFI_MATRIX_N_X_N_234_H__
...
// row is listed first in fetching
template<uint32,uint32> struct matrix;

template<uint32 row,uint32 col> inline float32 Determinant( const matrix<row,col>& );
template<uint32 row,uint32 col> inline matrix<col,row> Transpose( const matrix<row,col>& );
...
struct matrix
{
	...
	static matrix<row,col> identity() {
		InfiPushFunction( "matrix.identity" );
		if ( row != col ) {
			InfiSendError( INFI_TYPE_ERROR,
						   "Cannot set non-square matrix (%d x %d) to identity", row, col ); 
	...
	matrix<row,col> minors() const {
		...
		for ( uint32 i=0;i<row;i++ )
			for ( uint32 j=0;j<col;j++ )
				matret(i,j) = Determinant<row-1,col-1>( submatrix(i,j) );
		return matret;
	}
	...
	matrix<row,col> cofactors() const {
		...
		for ( uint32 i=0;i<row;i++ )
			for ( uint32 j=0;j<col;j++ )
				matret(i,j) = std::pow(-1,int32(i+j)) * Determinant<row-1,col-1>( submatrix(i,j) );
		return matret;
	}
	
	matrix<col,row> inverse() const {
		matrix<col,row> adjugate = Transpose<row,col>( cofactors() );
...
template<uint32 row,uint32 col>
inline float32 Determinant( const matrix<row,col>& mat ) {
	if ( row!=col )
		return 0;
	
	float32 fin = 0;
	for ( uint32 i=0;i<row;i++ )
		fin += pow( -1, int32(i) ) * mat(0,i) * Determinant<row-1,col-1>( mat.submatrix(0,i) );
		
	return fin;
}
template<>
inline float32 Determinant<2,2>( const matrix<2,2>& mat ) 
	{ return mat(0,0)*mat(1,1) - mat(0,1)*mat(1,0); }

template<>
inline float32 Determinant<1,1>( const matrix<1,1>& mat )
	{ return mat(0,0); }

template<uint32 row,uint32 col>
inline matrix<col,row> Transpose( const matrix<row,col>& mat ) {
...
inline mat2 CreateMatrix( float32 n00,float32 n01,float32 n10,float32 n11 ) {
	mat2 n;
...
inline mat3 ExpandMat3(const mat2& m) {
	mat3 n;
...
inline mat4 ExpandMat4(const mat2& m) {
	mat4 n;
	...
	n.data[3] =		 0; n.data[7] =		 0; n.data[11] = 1; n.data[15] = 0;
	return n;
}
	
inline void PrintMatrix( const mat2& mat ) { PrintMat<2,2>( mat ); }
inline float32 Determinant( const mat2& mat ) { return Determinant<2,2>(mat); }
inline mat2 Transpose( const mat2& mat ) { return Transpose<2,2>(mat); }

inline mat3 CreateMatrix(  float32 n00,float32 n01,float32 n02,
...
inline mat4 ExpandMat4(const mat3& m) {
	mat4 n;
...
inline float32 Determinant( const mat3& mat ) { return Determinant<3,3>(mat); }
inline mat3 Transpose( const mat3& mat ) { return Transpose<3,3>(mat); }

inline mat4 CreateMatrix(  float32 n00,float32 n01,float32 n02,float32 n03,
...
inline matrix<3,3> ReduceMatrix( const mat4& n ) {
	matrix<3,3> m;
...
inline float32 Determinant( const mat4& mat ) { return Determinant<4,4>(mat); }
inline mat4 Transpose( const mat4& mat ) { return Transpose<4,4>(mat); }


} }

#endif//__INFI_MATRIX_N_X_N_234_H__
