#ifndef __INFI_RECTANGLE_H__
#define __INFI_RECTANGLE_H__
...
union rect_t {
	...
	rect_t( __T nw, __T nh ) : x(0),y(0),w(nw),h(nh) { }
	template<typename __K> rect_t( const vec2_t<__K>& wh ) : x(0),y(0),w(wh.x),h(wh.y) { }
	rect_t( __T nx, __T ny, __T nw, __T nh ) : x(nx),y(ny),w(nw),h(nh) { }
	template<typename __K> rect_t( const vec2_t<__K>& xy, vec2_t<__K> wh ) : x(xy.x),y(xy.y),w(wh.x),h(wh.y) { }
	template<typename __K> rect_t( const vec4_t<__K>& all ) : x(all.x),y(all.y),w(all.z),h(all.w) { }
	rect_t( const rect_t& copy ) : data( copy.data ) { ; }
